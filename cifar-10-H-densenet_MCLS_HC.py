...
def train(model, trainloader, criterion, optimizer, threshold):
    ...
    for batch_idx, (inputs, targets, ad) in enumerate(trainloader):
        ...
        loss = criterion(outputs[ann_diff>threshold], targets[ann_diff>threshold], conf_score)
        loss.backward()
        optimizer.step()
...
for epoch in range(args.num_epoch):
    ...
    if accuracy > best_acc:
        ...
        torch.save(best_model.state_dict(), 'best_model_densenet_cifar10h_MCLS_HC.pth.tar')
...
model.load_state_dict(torch.load('best_model_densenet_cifar10h_MCLS_HC.pth.tar'))
...
write_csv("cifar10h_densenet_McLS_HC.csv", ["init_prob:" + str(args.initial_prob), "epoch_iid:" + str(args.epoch_iid),
