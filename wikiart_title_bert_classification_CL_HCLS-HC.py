...
parser.add_argument('--initial_prob', default = 0.3, type=float, help='initial prob')
parser.add_argument('--epoch_iid',default = 70, type=int, help='epoch iid')
...
class loss_fn_CL(torch.nn.Module):
    
    def __init__(self, classes= num_classes, smoothing=0.16, ignore_index=-1):
        super(loss_fn_CL, self).__init__()
...
def train_epoch(model, data_loader, loss_fn, optimizer, scheduler, device, n_examples,threshold):
    model = model.train()
    ...
    for d in data_loader:
        ...
        if sum(ann_diff>threshold) == 0:
            continue
        loss = loss_fn(outputs[ann_diff>threshold], targets[ann_diff>threshold], ad[ann_diff>threshold])
...
loss_fn = loss_fn_CL(classes = num_classes).to(device)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_accuracy:
        torch.save(model.state_dict(), 'best_model_wikiart_title_CL_HCLS_HC.pth')
        best_accuracy = val_acc
        best_epoch = epoch
    print('epoch: {}  acc: {:.4f}  best epoch: {}  best acc: {:.4f}'.format(
            epoch, val_acc, best_epoch, best_accuracy))

# write_csv("wikiart_title_CL_HCLS_HC.csv", ["init_prob:" + str(args.initial_prob), "epoch_iid:" + str(args.epoch_iid),
