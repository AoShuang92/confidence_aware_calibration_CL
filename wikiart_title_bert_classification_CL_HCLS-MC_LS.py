...
def train_epoch(model, model_conf, data_loader, loss_fn, optimizer, scheduler, device, n_examples,threshold):
    ...
    for d in data_loader:
        ...
        if sum(confidence>threshold) == 0:
            continue
        loss = loss_fn(outputs[confidence>threshold], targets[confidence>threshold], ad[confidence>threshold])
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_accuracy:
        torch.save(model.state_dict(), 'best_model_wikiart_title_CL_HCLS_LS1.pth')
        best_accuracy = val_acc
        best_epoch = epoch
    print('epoch: {}  acc: {:.4f}  best epoch: {}  best acc: {:.4f}'.format(
            epoch, val_acc, best_epoch, best_accuracy))

write_csv("wikiart_title_CL_HCLS_MC_LS.csv", ["init_prob:" + str(args.initial_prob), "epoch_iid:" + str(args.epoch_iid),
