...
def train_epoch(model, data_loader, loss_fn, optimizer, scheduler, device, n_examples,threshold):
    ...
    for d in data_loader:
        ...
        if sum(ann_diff>threshold) == 0:
            continue
        loss = loss_fn(outputs[ann_diff>threshold], targets[ann_diff>threshold], conf_score)
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_accuracy:
        torch.save(model.state_dict(), 'best_model_wikiart_title_CL_MCLS_HC.pth')
        best_accuracy = val_acc
        best_epoch = epoch
    print('epoch: {}  acc: {:.4f}  best epoch: {}  best acc: {:.4f}'.format(
            epoch, val_acc, best_epoch, best_accuracy))

write_csv("wikiart_title_CL_MCLS_HC.csv", ["init_prob:" + str(args.initial_prob), "epoch_iid:" + str(args.epoch_iid),
